...
メールを転送するときには、本文として転送する方法と、message/rfc822形式を使用して転送する方法の二通りが選べます。message/rfc822形式を使用した場合、複数のメッセージが選択されていると全てのメッセージが転送されます。どちらの形式を使用するかは、((<[オプション]ダイアログの[その他2]パネル|URL:OptionMisc2.html>))で指定できます。
