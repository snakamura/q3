...
labelエレメントは、ラベル設定コマンドを指定します。type属性が指定されなかった場合にはlabel属性で指定されたラベルを設定します。type属性にaddを指定するとlabel属性で指定されたラベルを追加し、removeを指定すると削除します。
...
applyエレメントはマクロの実行を指定します。対象のメッセージに対してmacro属性で指定されたマクロを実行します。
