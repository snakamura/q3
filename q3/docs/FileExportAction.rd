...
+[テンプレート]
エクスポートするときに適用するテンプレートを指定します。「（なし）」以外を指定すると、各メッセージに対して指定されたテンプレートを適用した結果を書き出します。この場合には、書き出すエンコーディングを[エンコーディング]で指定する必要があります。「（なし）」を指定した場合には、メッセージ自体がmessage/rfc822形式でエクスポートされます。
