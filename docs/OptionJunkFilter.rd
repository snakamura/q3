...
+[添付ファイルをスキャンする]
添付ファイルをスキャンするかどうかを指定します。添付ファイルをスキャンするには、xdoc2txtをインストールする必要があります。詳細は、((<スパムフィルタ|URL:JunkFilter.html>))を参照してください。デフォルトではスキャンしません。


+[最大サイズ]
添付ファイルをスキャンするときの最大サイズを指定します。ここで指定したサイズよりも大きな添付ファイルはスキャンされません。デフォルトは、32768(32KB)です。
