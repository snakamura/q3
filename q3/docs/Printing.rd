...
別のアプリケーションで印刷したい場合には、保存されるファイルのqmail.xmlのGlobal/PrintExtensionで拡張子を指定することができます。ここで拡張子を指定した上で、その拡張子に対して印刷の関連付けを行ってください。また、デフォルトのprint.templateはメッセージをHTMLにフォーマットするようになっていますので、適宜書き換える必要があります。
