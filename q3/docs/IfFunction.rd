...
==説明
condition評価してTrueになったときにはvalueを返します。Falseになったときにはotherwiseを返します。3個以上の任意の奇数個の引数を渡せます。この場合、1番目の引数がTrueになった場合には2番目の引数を、3番目の引数がTrueになった場合には4番目の引数を、というように評価していき、どの条件にも合致しなかった場合には最後の引数を返します。
