...
showAlways属性にはtrueまたはfalseを指定します。trueを指定するとコンテキストアカウントがない場合でも常にアイテムに文字列を表示します。この場合、テンプレート中にマクロを書くことはできません。指定しない場合にはfalsegを指定した場合と同じになります。
...
editエレメントはエディットコントロールを表します。ただし編集できるわけではなく、スタティックコントロールとの主な違いは自動スクロールするだけです。コンテンツにテンプレート書式で表示する文字列を指定します。

name, width, showAlways, background, style, align属性についてはstaticエレメントを参照してください。
...
name, width, showAlways, background属性についてはstaticエレメントを参照してください。
