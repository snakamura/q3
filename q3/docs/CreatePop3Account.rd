...
[名前]には任意のアカウント名を指定します。ただし、ファイル名として使えない文字は使えません。また、正規表現のメタキャラクタ（()[]{}.*+?$など）は使わないほうが後々トラブルが発生する可能性が少なくなります。
...
SMTP認証を使う場合には[送信]の[認証]にチェックを入れ、[ユーザ名]と[パスワード]を同様に指定します。POP3 before SMTPを使う場合には、[送信]の[認証]にはチェックを入れません。
...
受信したメッセージをサーバから削除する場合には[サーバからメッセージを削除]にチェックを入れます。また、受信してから一定時間以上経過したメッセージをサーバから削除する場合には[?日たったらサーバからメッセージを削除]に0以外の日数を指定します。

その他の設定については、((<POP3の設定|URL:AccountPop3.html>))を参照してください。


[SMTP]タブは飛ばして、[ダイアルアップ]タブをクリックします。
...
((<[高度]タブ|"IMG:images/TutorialPop3AdvancedPage.png">))

POP3 before SMTPを使用するには、[送信前に受信サーバに接続する]にチェックを入れます。

その他の設定ついては、((<高度の設定|URL:AccountAdvanced.html>))を参照してください。
