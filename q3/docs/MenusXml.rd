...
静的なアイテムは定義時に決まっているアイテムで、text属性、action属性、param属性を使って指定します。text属性には表示する文字列を指定します。action属性にはそのアイテムが選択されたときに実行される((<アクション|URL:Action.html>))を指定します。param属性には必要があればアクションに渡す引数を指定します。引数はスペースで区切って複数指定することができます。一つの引数にスペースを含めたい場合には""で括ります。
...
これに加えて、以下のアクションをdynamic属性に指定し、param属性を省略すると説明にあるようなメニューが動的に生成されます。

:AddressCreateMessage
  選択されているアドレス帳のエントリに登録されているアドレスをリストします
:MessageCreate
  ファイル名がcreate_から始まる作成用テンプレートをリストします
...
:ViewSort
  リストビューのカラムをリストします
:ViewTemplate
  ファイル名がview_から始まる表示用テンプレートをリストします
