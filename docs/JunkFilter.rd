...
==添付ファイルのスキャン
PDFなどを添付した上でその中にスパムの文言が記述されているスパムに対応するために、添付ファイルの中身をスキャンすることができます。添付ファイルをスキャンするためには、まず添付ファイルからテキストを取り出すために、((<xdoc2txt|URL:http://www31.ocn.ne.jp/%7Eh_ishida/xdoc2txt.html>))をインストールする必要があります。リンク先のページからxdoc2txtの最新版と、同じページにあるcryptlib.dllの最新版をダウンロードし、適当なディレクトリにインストールし、そのディレクトリにPATHを通してください。

xdoc2txtがインストールできたら、((<スパムフィルタの設定|URL:OptionJunkFilter.html>))で、[添付ファイルをスキャンする]にチェックを入れます。
...
==設定
((<スパムフィルタの設定|URL:OptionJunkFilter.html>))を参照してください。
...
==その他
対象のアカウントにスパムフォルダがないとスパムフィルタが働きません。POP3やNNTPアカウントではこのフォルダは自動的に作成されますが、IMAP4アカウントでは自動的に作成されません。また、Windows CE版でアカウントを作成するとスパムフォルダが作成されません。このような場合には、スパムを格納するためのフォルダを作成し、スパムフラグを設定する必要があります。詳細は、((<フォルダのプロパティ|URL:FolderProperty.html>))を参照してください。
