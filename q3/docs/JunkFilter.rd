...
==その他
対象のアカウントにスパムフォルダがないとスパムフィルタが働きません。POP3やNNTPアカウントではこのフォルダは自動的に作成されますが、IMAP4アカウントでは自動的に作成されません。また、Windows CE版でアカウントを作成するとスパムフォルダが作成されません。このような場合には、スパムを格納するためのフォルダを作成し、スパムフラグを設定する必要があります。詳細は、((<FolderPropertyアクション|URL:FolderPropertyAction.html>))を参照してください。
