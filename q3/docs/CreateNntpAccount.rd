...
[NNTP (POST)]タブは設定項目がないので飛ばします。[ダイアルアップ]と[高度]タブはPOP3アカウントと同様に指定します。
...
[名前]にはグループ名をしていします。[タイプ]は[ローカルフォルダ]を指定し、[同期可能]にチェックが入っていることを確認してください。[OK]をクリックするとフォルダが作成されます。

同期すると、指定したグループのメッセージが作成したフォルダに格納されます。
