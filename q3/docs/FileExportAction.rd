...
+[テンプレート]
エクスポートするときに適用するテンプレートを指定します。「（なし）」以外を指定すると、各メッセージに対して指定されたテンプレートを適用した結果を書き出します。この場合には、書き出すエンコーディングを[エンコーディング]で指定する必要があります。「（なし）」を指定した場合には、メッセージ自体が((<"message/rfc822形式"|URL:URL:http://www.ietf.org/rfc/rfc2046.txt>))でエクスポートされます。

ここで使用されるテンプレートは、テンプレート名が「export_」から始まっている必要があります。
